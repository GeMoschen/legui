...
public class NvgFBOImageRenderer extends NvgImageRenderer<FBOImage> {
     ...
     */
    @Override
    protected void renderImage(FBOImage image, Vector2fc position, Vector2fc size, Map<String, Object> properties, Context context, NvgContext nvgContext) {
        ...
        try (NVGPaint imagePaint = NVGPaint.calloc()) {
            ...
            int imageRef = manager.getImageReference(image, nvgContext.getId());
            float x = position.x();
            float y = position.y();
            float w = size.x();
            float h = size.y();
            ...
            nvgImagePattern(nvgContext.getId(), x, y, w, h, 0, imageRef, 1, imagePaint);
            nvgRoundedRectVarying(nvgContext.getId(), x, y, w, h, r.x, r.y, r.z, r.w);
            nvgFillPaint(nvgContext.getId(), imagePaint);
